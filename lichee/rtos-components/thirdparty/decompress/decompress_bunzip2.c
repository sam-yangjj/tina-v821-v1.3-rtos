...
static unsigned int get_bits(struct bunzip_data *bd, char bits_wanted)
{
	...
	bits_wanted = MIN(bits_wanted, sizeof(unsigned int) * CHAR_BIT - 1);
	unsigned int bitmask = (1u << bits_wanted) - 1u; 
	bits |= (bd->inbufBits >> bd->inbufBitCount)&bitmask;
...
static int read_bunzip(struct bunzip_data *bd, unsigned char *outbuf, int len)
{
...
/* Example usage: decompress src_fd to dst_fd.  (Stops at end of bzip2 data,
   not end of file.) */
STATIC int bunzip2(unsigned char *buf, long len,
			long (*fill)(void*, unsigned long),
...
static inline int __decompress(unsigned char *buf, long len,
			long (*fill)(void*, unsigned long),
			long (*flush)(void*, unsigned long),
