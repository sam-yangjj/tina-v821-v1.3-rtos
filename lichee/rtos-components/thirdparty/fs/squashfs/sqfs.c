...
static int sqfs_disk_read(__u32 block, int nr_blocks, void *buf)
{
	int ret;

	ret = flash_read(ctxt.start + block, nr_blocks, buf);
	if (ret) {
		printf("flash_read: blk:%d nblk:%d failed, ret=%d\n", block, nr_blocks, ret);
		return -1;
	}
...
{
	*sblk = malloc_cache_aligned(ctxt.blksz);
	if (!*sblk)
		return -ENOMEM;

	if (sqfs_disk_read(0, 1, *sblk) != 1) {
		free_cache_aligned(*sblk);
		*sblk = NULL;
		return -EINVAL;
	}
...
static int sqfs_calc_n_blks(__le64 start, __le64 end, u64 *offset)
{
	...
	start_ = le64_to_cpu(start) / ctxt.blksz;
	*offset = le64_to_cpu(start) - (start_ * ctxt.blksz);

	return DIV_ROUND_UP(table_size + *offset, ctxt.blksz);
}
			    ...
			    struct squashfs_fragment_block_entry *e)
{
	...
	table = malloc_cache_aligned(n_blks * ctxt.blksz);
	if (!table) {
		ret = -ENOMEM;
		goto out;
	}
	...
	entries = sqfs_malloc(SQFS_METADATA_BLOCK_SIZE);
	if (!entries) {
		ret = -ENOMEM;
		goto out;
	}
...
out:
	sqfs_free(entries);
	free_cache_aligned(metadata_buffer);
	free_cache_aligned(table);
...
static int sqfs_read_entry(struct squashfs_directory_entry **dest, void *src)
{
	...
	if (*dest)
		sqfs_free(*dest);
	*dest = sqfs_malloc(sizeof(*tmp) + sz + 2);
	if (!*dest)
		return -ENOMEM;
...
static char *sqfs_get_abs_path(const char *base, const char *rel)
{
	...
	base_tokens = sqfs_calloc(bc, sizeof(char *));
	if (!base_tokens)
		return NULL;

	rel_tokens = sqfs_calloc(rc, sizeof(char *));
	if (!rel_tokens)
		goto out;
	...
	resolved = sqfs_malloc(resolved_size + 1);
	if (!resolved)
		goto out;
...
out:
	if (rel_tokens)
		for (i = 0; i < rc; i++)
			sqfs_free(rel_tokens[i]);
	if (base_tokens)
		for (i = 0; i < bc; i++)
			sqfs_free(base_tokens[i]);

	sqfs_free(rel_tokens);
	sqfs_free(base_tokens);
			   ...
			   int token_count, u32 *m_list, int m_count)
{
	...
	struct squashfs_dir_stream *dirsp;
	struct squashfs_dirent *dent;
	unsigned char *table;
	...
	dirsp = (struct squashfs_dir_stream *)dirs;
	...
	dirs->dir_header = sqfs_malloc(SQFS_DIR_HEADER_SIZE);
	if (!dirs->dir_header)
		return -ENOMEM;
	...
	for (j = 0; j < token_count; j++) {
		...
		if (get_unaligned_le16(&dir->inode_type) == SQFS_SYMLINK_TYPE) {
			...
			res = sqfs_malloc(strlen(rem) + strlen(target) + 1);
			if (!res) {
				ret = -ENOMEM;
				goto out;
			}
			...
			sym_tokens = sqfs_malloc(token_count * sizeof(char *));
			if (!sym_tokens) {
				ret = -EINVAL;
				goto out;
			}
...
static int sqfs_read_inode_table(unsigned char **inode_table)
{
	...
	itb = malloc_cache_aligned(n_blks * ctxt.blksz);
	if (!itb)
		return -ENOMEM;
	...
	*inode_table = sqfs_malloc(metablks_count * SQFS_METADATA_BLOCK_SIZE);
	if (!*inode_table) {
		ret = -ENOMEM;
	...
	memset(*inode_table, 0, metablks_count * SQFS_METADATA_BLOCK_SIZE);
...
static int sqfs_read_directory_table(unsigned char **dir_table, u32 **pos_list)
{
	...
	*pos_list = NULL;
	...
	dtb = malloc_cache_aligned(n_blks * ctxt.blksz);
	if (!dtb)
		return -ENOMEM;
	...
	*dir_table = sqfs_malloc(metablks_count * SQFS_METADATA_BLOCK_SIZE);
	if (!*dir_table)
		goto out;

	*pos_list = sqfs_malloc(metablks_count * sizeof(u32));
	if (!*pos_list)
		goto out;
...
int sqfs_opendir(const char *filename, struct squashfs_dir_stream **dirsp)
{
	...
	dirs = sqfs_calloc(1, sizeof(*dirs));
	if (!dirs)
		return -EINVAL;
	...
	metablks_count = ctxt.metablks_count;
	/* Tokenize filename */
	...
	token_list = sqfs_malloc(token_count * sizeof(char *));
	if (!token_list) {
		ret = -EINVAL;
		goto out;
	}
	 ...
	 */
	dirs->inode_table = ctxt.inode_table;
	dirs->dir_table = ctxt.dir_table;
	ret = sqfs_search_dir(dirs, token_list, token_count, ctxt.pos_list,
			      metablks_count);
	if (ret)
		goto out;
...
out:
	for (j = 0; j < token_count; j++)
		sqfs_free(token_list[j]);
	sqfs_free(token_list);
	sqfs_free(path);
	if (ret) {
		sqfs_free(dirs);
	}
...
int sqfs_readdir(struct squashfs_dir_stream *fs_dirs, struct squashfs_dirent **dentp)
{
	...
	name_size = min_t(u16, dirs->entry->name_size + 1, SQFS_DIRENT_NAME_LEN - 1);
...
int sqfs_probe(u32 start, u32 size, u32 blksz)
{
	...
	ret = sqfs_read_inode_table(&ctxt.inode_table);
	if (ret) {
		goto error;
	}

	ret = sqfs_read_directory_table(&ctxt.dir_table, &ctxt.pos_list);
	if (ret < 1) {
		ret = -EINVAL;
		goto error;
	}
	ctxt.metablks_count = ret;

	return 0;
error:
	if (ctxt.dir_table)
		sqfs_free(ctxt.dir_table);
	if (ctxt.pos_list)
		sqfs_free(ctxt.pos_list);
	if (ctxt.inode_table)
		sqfs_free(ctxt.inode_table);
	if (ctxt.sblk)
		free_cache_aligned(ctxt.sblk);
	ctxt.sblk = NULL;
	return ret;
}
...
static int sqfs_split_path(char **file, char **dir, const char *path)
{
	...
	*dir = NULL;
	...
	if (path[0] == '/') {
		...
		tmp_path = sqfs_malloc(strlen(path) + 2);
		if (!tmp_path) {
			ret = -ENOMEM;
			goto out;
		}
				 ...
				 __le32 blksz)
{
	...
	if (datablk_count) {
		finfo->blk_sizes = sqfs_malloc(datablk_count * sizeof(u32));
		if (!finfo->blk_sizes)
			return -ENOMEM;
	}
...
int sqfs_read(const char *filename, void *buf, u32 offset, u32 len,
	      u32 *actread)
{
	...
	struct squashfs_dir_stream *dirsp = NULL;
	struct squashfs_dir_stream *dirs;
	struct squashfs_lreg_inode *lreg;
	struct squashfs_base_inode *base;
	struct squashfs_reg_inode *reg;
	...
	*actread = 0;
	...
	case SQFS_REG_TYPE:
		...
		datablk_count = sqfs_get_regfile_info(reg, &finfo, &frag_entry,
						      sblk->block_size);
		if (datablk_count < 0) {
			printf("get regfile info failed, ret=%d\n", ret);
			ret = -EINVAL;
			goto out;
		}
	...
	if (datablk_count) {
		...
		datablock = sqfs_malloc(get_unaligned_le32(&sblk->block_size));
		if (!datablock) {
			ret = -ENOMEM;
			goto out;
		}
	...
	for (j = 0; j < datablk_count; j++) {
		...
		start = data_offset / ctxt.blksz;
		table_size = SQFS_BLOCK_SIZE(finfo.blk_sizes[j]);
		table_offset = data_offset - (start * ctxt.blksz);
		...
		if (finfo.blk_sizes[j] == 0) {
			...
			data_buffer = malloc_cache_aligned(n_blks * ctxt.blksz);
		...
		free_cache_aligned(data_buffer);
		if (*actread >= len)
			break;
	}
	...
	start = frag_entry.start / ctxt.blksz;
	table_size = SQFS_BLOCK_SIZE(frag_entry.size);
	table_offset = frag_entry.start - (start * ctxt.blksz);
	n_blks = DIV_ROUND_UP(table_size + table_offset, ctxt.blksz);

	fragment = malloc_cache_aligned(n_blks * ctxt.blksz);

	if (!fragment) {
		printf("malloc_cache_aligned %lld bytes for fragment failed!\n", n_blks * ctxt.blksz);
		ret = -ENOMEM;
		goto out;
	}
	...
	if (finfo.frag && finfo.comp) {
		dest_len = get_unaligned_le32(&sblk->block_size);
		fragment_block = sqfs_malloc(dest_len);
		if (!fragment_block) {
			ret = -ENOMEM;
			goto out;
		}
...
void sqfs_closedir(struct squashfs_dir_stream *dirs)
{
	...
	sqfs_free(sqfs_dirs->dir_header);
	if (sqfs_dirs->entry)
		sqfs_free(sqfs_dirs->entry);
	sqfs_free(sqfs_dirs);
}
