...
#ifdef CONFIG_AW_RPMSG_PERF_TRACE
#define RPMSG_HEADER_SIZE_ALIGN(x) ((x + (64 - 1)) & (64 - 1))
...
{
	rpmsg_perf_record_ts_in_ept(ept, RPMSG_TRACE_R_END);
}
#else
static inline void rpmsg_perf_set_ts_in_header(struct rpmsg_hdr *hdr, int idx, uint64_t timestamp) { }
static inline void rpmsg_perf_record_ts_in_header(struct rpmsg_hdr *hdr, int idx) { }
...
#ifdef CONFIG_AW_RPMSG_PERF_TRACE
	memset(rp_hdr.trace, 0, sizeof(rp_hdr.trace));
	copy_sender_ts_to_header(&rp_hdr, ept);
#endif
...
#ifdef CONFIG_AW_RPMSG_PERF_TRACE
	rpmsg_perf_record_ts_in_header(hdr, RPMSG_TRACE_S_NOTIFY);
#ifdef VIRTIO_CACHED_BUFFERS
	metal_cache_flush(hdr, RPMSG_HEADER_SIZE_ALIGN(sizeof(struct rpmsg_hdr)));
#endif /* VIRTIO_CACHED_BUFFERS */
#endif
...
#ifdef CONFIG_AW_RPMSG_PERF_TRACE
	rpmsg_perf_record_ts_in_header(hdr, RPMSG_TRACE_S_END);
#ifdef VIRTIO_CACHED_BUFFERS
	metal_cache_flush(hdr, RPMSG_HEADER_SIZE_ALIGN(sizeof(struct rpmsg_hdr)));
#endif /* VIRTIO_CACHED_BUFFERS */
#endif
					    ...
					    int len, int wait)
{
	...
	rpmsg_perf_set_ts_in_ept(ept, RPMSG_TRACE_S_START, timestamp);
	rpmsg_perf_record_ts_in_ept(ept, RPMSG_TRACE_S_FILL);
	...
	return rpmsg_virtio_send_offchannel_nocopy(rdev, ept, src, dst, buffer, len);
}
...
static void rpmsg_virtio_rx_callback(struct virtqueue *vq)
{
	...
	while (rp_hdr) {
		...
		if (ept) {
			...
			copy_sender_ts_to_ept(ept, rp_hdr);
			rpmsg_perf_set_ts_in_ept(ept, RPMSG_TRACE_R_PREPARE, timestamp);
			rpmsg_perf_record_ts_in_ept(ept, RPMSG_TRACE_R_CB);
			...
			if (ret) {
				openamp_err("get rpmsg performance data failed, ret: %d\n", ret);
			} else {
				openamp_info("'%s' performance data:\n", ept->name);
...
#ifdef CONFIG_AW_RPMSG_PERF_TRACE
	int ret = amp_ts_get_dev(CONFIG_RPMSG_AMP_TS_DEV_ID, &g_rpmsg_amp_ts_dev);
	if (ret) {
		openamp_err("get AMP timestamp device failed, ret: %d, rpmsg performance data is invalid!\n", ret);
	}
#endif
