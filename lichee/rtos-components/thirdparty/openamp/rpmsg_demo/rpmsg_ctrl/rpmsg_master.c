...
struct rpmsg_ept_ctrldev {
	...
	int is_init;
	struct rpmsg_mutex_t list_lock;
	hal_thread_t daemon;
...
static int rpmsg_mutex_init(struct rpmsg_mutex_t *lock)
{
	if (lock->is_init)
		return 0;

	lock->is_init = 1;
	hal_mutex_init(&lock->lock);
	lock->ref = 0;
	lock->owner = NULL;
	return 0;
}

static int rpmsg_mutex_deinit(struct rpmsg_mutex_t *lock)
{
	hal_assert(lock->ref == 0);

	hal_mutex_detach(&lock->lock);
...
static int rpmsg_mutex_lock(struct rpmsg_mutex_t *lock)
{
	int ret = 0;

	hal_assert(lock->ref >= 0);

	if (lock->owner != hal_thread_self()) {
		ret = hal_mutex_lock(&lock->lock);
		if (ret)
			goto out;
		lock->owner = hal_thread_self();
	}
	lock->ref++;
out:
	return ret;
...
static int rpmsg_mutex_unlock(struct rpmsg_mutex_t *lock)
{
	int ret = 0;

	hal_assert(lock->owner == hal_thread_self());
	hal_assert(lock->ref > 0);

	lock->ref--;
	if (!lock->ref) {
		ret = hal_mutex_unlock(&lock->lock);
		if (ret)
			goto out;
		lock->owner = NULL;
	}
...
{
	rpmsg_mutex_lock(&monitor->lock);
	list_add(&client->monitor, &monitor->epts);
	rpmsg_mutex_unlock(&monitor->lock);
}
...
{
	rpmsg_mutex_lock(&ctrldev.list_lock);
	list_add(&client->list, &ctrldev.epts_list);
	rpmsg_mutex_unlock(&ctrldev.list_lock);
}
...
rpmsg_ctrl_alloc_client(struct rpmsg_ctrl_msg *msg, struct rpmsg_monitor_entry *monitor)
{
	...
	if (strlen(msg->name) >= RPMSG_MAX_NAME_LEN) {
		hal_log_err("%s is too long, require less than %d\r\n", msg->name, RPMSG_MAX_NAME_LEN);
		return NULL;
	}
				...
				rpmsg_func_cb unbind, uint32_t cnt, void *priv)
{
	...
	if (ctrldev.is_init != 1) {
		hal_log_warn("ctrldev not create\r\n");
		return -ENXIO;
	}
...
void rpmsg_client_unbind(const char *name)
{
	...
	rpmsg_mutex_deinit(&monitor->lock);
	hal_free(monitor);
}
...
int rpmsg_ctrldev_create(void)
{
	...
	rpmsg_mutex_init(&ctrldev.list_lock);
