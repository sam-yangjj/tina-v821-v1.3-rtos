...
#endif
	flag = 1;
	nRetCode = setsockopt(pNetworkContext->Socket, IPPROTO_TCP, TCP_NODELAY, (void *)&flag,
								   ( socklen_t ) sizeof(flag));
	if (nRetCode < 0) {
		NETWORK_ERR(("%s setsocket TCP_NODELAY failed.\n", __func__));
	}

	nRetCode = setsockopt(pNetworkContext->Socket, IPPROTO_TCP, TCP_QUICKACK,(void *) &flag,
								   ( socklen_t ) sizeof(flag));
	if (nRetCode < 0) {
		NETWORK_ERR(("%s setsocket TCP_QUICKACK failed.\n", __func__));
	}
                                     ...
                                     size_t bytesToSend )
{
	...
	NETWORK_DBG(("%s wanto send bytes: %d send...\n",__func__, bytesToSend));
