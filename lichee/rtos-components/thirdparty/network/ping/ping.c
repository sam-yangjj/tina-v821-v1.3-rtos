...
//	fcntl(iSockID, F_SETFL, O_NONBLOCK);  /* set noblocking */
			...
			if (iStatus > 0 && FD_ISSET(iSockID, &ReadFds)) {
				...
				if (reply_size >= (int)(sizeof(struct ip_hdr)+sizeof(struct icmp_echo_hdr))) {
					...
					if ((pecho->id == PING_IDs) && (pecho->seqno == htons(ping_seq_num))) {
						...
						printf("%d bytes from %s: icmp_seq=%d	 time=%d ticks\n",
						       (uint32_t)(reply_size - (IPH_HL(iphdr) * 4) - sizeof(struct icmp_echo_hdr)),
						       inet_ntoa(FromAddr.sin_addr),
						       htons(pecho->seqno), (uint32_t)TimeElapse);
	...
	printf("rtt min/avg/max = %d/%d/%dus\n\n",
			(uint32_t)OS_TicksToUSecs(min_rtt),
			(uint32_t)OS_TicksToUSecs(time_counter/data->count),
			(uint32_t)OS_TicksToUSecs(max_rtt));
