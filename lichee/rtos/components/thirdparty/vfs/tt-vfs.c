...
#ifdef CONFIG_COMPONENTS_RPCFS
	extern int rpcfs_register_vfs(const char *mnt_path);
    ret = rpcfs_register_vfs(CONFIG_COMPONENTS_RPCFS_PATH);
    if (ret) {
        printf("register rpcfs failed\n");
        goto err;
    }
#endif
...
FINSH_FUNCTION_EXPORT_CMD(vfs_test, vfs, test vfs operation);
