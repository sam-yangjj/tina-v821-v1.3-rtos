...
#ifdef CONFIG_KASAN_PLATFORM_CHECK
    if (platform_check_memory_region_invalid_inline(addr) != 0) {
	return;
    }
#else
...
{
    PLATFORM_KASAN_CHECK_ADDR_FILTER(global->beg);
...
#define DEFINE_ASAN_LOAD_STORE(size)                    \
    ...
    {                               \
        PLATFORM_KASAN_CHECK_ADDR_FILTER(addr);\
