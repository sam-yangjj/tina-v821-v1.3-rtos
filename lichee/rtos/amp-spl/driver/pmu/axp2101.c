...
static axp_contrl_info axp_ctrl_tbl[] = {
	...
	{ "dcdc5", 1400, 3700, AXP2101_DC5OUT_VOL, 0x1f, AXP2101_OUTPUT_CTL0, 4, 0,
	{ {1400, 3700, 100}, } },
...
int axp2101_set_avdd_voltage(int set_vol, int onoff)
{
	u8 value;
	pmic_bus_read(AXP2101_RUNTIME_ADDR, AXP2101_BLDO2OUT_VOL, &value);
	printf("AXP2101_BLDO2OUT_VOL = 0x%x\n", value);
	printf("set bldo2 to %d\n", set_vol);
	return pmu_set_vol("bldo2", set_vol, onoff);
}


int axp2101_set_dvdd_voltage(int set_vol, int onoff)
{
	printf("set dcdc5 to %d\n", set_vol);
	return pmu_set_vol("dcdc5", set_vol, onoff);
}

int axp2101_set_iovdd_voltage(int set_vol, int onoff)
{
	printf("set bldo1 to %d\n", set_vol);
...
int axp2101_axp_init(u8 power_mode)
{
	...
	if (pmic_bus_init(AXP2101_DEVICE_SPEED)) {
