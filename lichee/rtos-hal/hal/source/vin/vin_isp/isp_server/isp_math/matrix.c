...
Matrix *create_matrix(int lines, int columns)
{
	...
	// set matrix's value
	a->value = hal_malloc(lines * sizeof(float *));
	if (a->value == NULL) {
		printf("a->value malloc fail\n");
		hal_free(a);
		return NULL;
	}
	memset(a->value, 0, lines * sizeof(float *));

	for (i = 0; i < lines; i++) {
		a->value[i] = hal_malloc(columns * sizeof(float));
		if (a->value[i] == NULL) {
			printf("a->value[i] malloc fail\n");
			return NULL;
		}
		memset(a->value[i], 0, columns * sizeof(float));
	}
