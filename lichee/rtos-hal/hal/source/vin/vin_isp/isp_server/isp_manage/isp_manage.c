...
void __isp_rolloff_run(struct isp_lib_context *isp_gen)
{
	...
	if (!isp_gen->isp_ini_cfg.isp_test_settings.wdr_stitch_en) {
		...
		for (i = 0; i < 256; i++) {
			lens_tbl[4*i + 0] = (unsigned short)sqrtf((float)isp_rolloff_cxt->rolloff_result.lens_table_output[i + 0] * 1024);
			lens_tbl[4*i + 1] = (unsigned short)sqrtf((float)isp_rolloff_cxt->rolloff_result.lens_table_output[i + 256] * 1024);
			lens_tbl[4*i + 2] = (unsigned short)sqrtf((float)isp_rolloff_cxt->rolloff_result.lens_table_output[i + 512] * 1024);
		}
	}
}
...
void __isp_pltm_set_params(struct isp_lib_context *isp_gen)
{
	...
	isp_gen->pltm_entity_ctx.pltm_param->adc_fast_convergence = 0;
#endif
	isp_gen->pltm_entity_ctx.pltm_param->lft_use_hfr_stren = PLTM_LFT_USE_HFR_STREN;
...
void __isp_ctx_cfg_lib(struct isp_lib_context *isp_gen)
{
	...
	isp_gen->sensor_otp.otp_info = NULL;
	isp_gen->sensor_otp.pmsc_table = NULL;
	isp_gen->sensor_otp.pwb_table = NULL;
	isp_gen->sensor_otp.paf_table = NULL;
}
...
HW_S32 __isp_ctx_update_iso_cfg(struct isp_lib_context *isp_gen)
{
	...
	if (isp_gen->sensor_info.fps_fixed > 60) {//HRF
		...
		isp_gen->iso_entity_ctx.iso_param->shading_adjust = false;
		isp_gen->iso_entity_ctx.iso_param->encoder_adjust = false;
#if (ISP_VERSION >= 602)
		isp_gen->iso_entity_ctx.iso_param->awb_adjust = true;
#endif
	}
...
HW_S32 __isp_ctx_update_ae_cfg(struct isp_lib_context *isp_gen)
{
	...
	//Use for fastboot high frame rate
	if (isp_gen->isp_ini_cfg.isp_3a_settings.ae_reserve_4) {
		isp_gen->ae_entity_ctx.ae_param->hfr_fast_algo_en = 1;
		isp_gen->ae_entity_ctx.ae_param->hfr_cal_num = ISP_FAST_CONVERGENCE_ALGO_FRAME;
	} else {
		isp_gen->ae_entity_ctx.ae_param->hfr_fast_algo_en = 0;
		isp_gen->ae_entity_ctx.ae_param->hfr_cal_num = ISP_FAST_CONVERGENCE_FRAME;
	}
	return 0;
}
