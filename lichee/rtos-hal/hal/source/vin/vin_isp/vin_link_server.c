...
static int isp_config_sensor_info(struct isp_dev *isp)
{
	...
	ctx->sensor_info.sensor_width = sensor_format->width & 0x0000ffff;
	if (isp->large_image) {
		ctx->sensor_info.width_overlayer = (sensor_format->width & 0x0000ffff) / 2 + vin_set_large_overlayer(sensor_format->width);
	} else {
		ctx->sensor_info.width_overlayer = 0;
	}
...
static int isp_config_switch_sensor_info(struct isp_dev *isp)
{
	...
	if (global_sensors[sensor_id].sensor_core->sensor_g_switch_format)
		sensor_format = global_sensors[sensor_id].sensor_core->sensor_g_switch_format(sensor_id, vinc->isp_sel, vinc->id);
	else
		return -1;
...
void isp_set_load_reg(struct isp_dev *isp)
{
	...
	if (isp_gen->isp_load_flag) {
		if (isp->large_image && global_isp[0]) {
			if (isp->id == 0) {
				//ae stat region
				memcpy(&isp->ae_stat_save[0], isp_gen->load_reg_base + ISP_AE_REG_OFS, ISP_AE_REG_SIZE);
				//af stat region
				memcpy(&isp->af_stat_save[0], isp_gen->load_reg_base + ISP_AF_REG_OFS, ISP_AF_REG_SIZE);
				//awb stat region
				memcpy(&isp->awb_stat_save[0], isp_gen->load_reg_base + ISP_AWB_REG_OFS, ISP_AWB_REG_SIZE);
				//hist stat region
				memcpy(&isp->hist_stat_save[0], isp_gen->load_reg_base + ISP_HIST_REG_OFS, ISP_HIST_REG_SIZE);
				//nr msc table
				memcpy(&isp->nr_msc_load_save[0], isp_gen->load_reg_base + ISP_LOAD_REG_SIZE + ISP_FE_TBL_SIZE + ISP_RSC_TBL_SIZE + ISP_RSC_TBL_SIZE, ISP_MSC_NR_TBL_SIZE);
				//return ret;
				if (!isp->init_done_flag) {
					isp->load_save = 1;
					isp->init_done_flag = 1;
					hal_mutex_unlock(isp_gen->ctx_lock);
					return;
				}
			} else {
				//ISP_PRINT("isp%d set load\n", isp->id);
				isp->load_buf_busy = 1;
				memcpy(&isp->load_shadow[0], isp_gen->load_reg_base, ISP_LOAD_DRAM_SIZE);
				if (global_isp[0]->use_isp) {
					memcpy(&global_isp[0]->load_shadow[0], &isp->load_shadow[0], ISP_LOAD_DRAM_SIZE);
					//AHB Registers
					memcpy(&global_isp[0]->load_shadow[ISP_AHB0_MEM_OFS], &isp->load_shadow[ISP_AHB1_MEM_OFS], ISP_AHB_MEM_SIZE);
					//lsc table copy to msc table
					memcpy(&global_isp[0]->load_shadow[ISP_LOAD_REG_SIZE + ISP_FE_TBL_SIZE + ISP_RSC_TBL_SIZE],
							&isp->load_shadow[ISP_LOAD_REG_SIZE + ISP_FE_TBL_SIZE], ISP_RSC_TBL_SIZE);
					//nr msc copy
					memcpy(&global_isp[0]->load_shadow[ISP_LOAD_REG_SIZE + ISP_FE_TBL_SIZE + ISP_RSC_TBL_SIZE + ISP_RSC_TBL_SIZE],
							&global_isp[0]->nr_msc_load_save[0], ISP_MSC_NR_TBL_SIZE);
					//ae stat region
					memcpy(&global_isp[0]->load_shadow[ISP_AE_REG_OFS], &global_isp[0]->ae_stat_save[0], ISP_AE_REG_SIZE);
					//af stat region
					memcpy(&global_isp[0]->load_shadow[ISP_AF_REG_OFS], &global_isp[0]->af_stat_save[0], ISP_AF_REG_SIZE);
					//awb stat region
					memcpy(&global_isp[0]->load_shadow[ISP_AWB_REG_OFS], &global_isp[0]->awb_stat_save[0], ISP_AWB_REG_SIZE);
					//hist stat region
					memcpy(&global_isp[0]->load_shadow[ISP_HIST_REG_OFS], &global_isp[0]->hist_stat_save[0], ISP_HIST_REG_SIZE);
					if (global_isp[0]->init_done_flag) {
						global_isp[0]->load_flag = 1;
					}
				} else {
...
int vin_server_init(struct isp_dev *isp, struct isp_part_para isp_part)
{
	...
	if (isp_part.ir_mode == 0x6)
		ir = 2;
	else if (isp_part.ir_mode == 0x2)
		ir = 1;
	else
		ir = 0;

	if (isp->large_image) {
		isp_gen->stitch_mode = STITCH_2IN1_LINNER;
		isp_server_set_sync((1 << 1) | (1 << 0));
	} else {
		isp_gen->stitch_mode = STITCH_NONE;
		isp_server_set_sync(0);
	}
...
int vin_isp_ae_done(struct isp_dev *isp, int drap_frame_en)
{
	...
	if (!drap_frame_en ||
			(ae_result->ae_status == AE_STATUS_DONE || isp_gen->ae_frame_cnt >= isp_gen->ae_entity_ctx.ae_param->hfr_cal_num)) {
