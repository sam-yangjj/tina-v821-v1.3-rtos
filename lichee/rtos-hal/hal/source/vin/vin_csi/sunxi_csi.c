...
static int __csi_set_fmt_hw(struct csi_dev *csi, unsigned int vinc_id)
{
	...
	for (i = 0; i < csi->bus_info.ch_total_num; i++) {
		if (csi->large_image == 1) {
			csic_prs_ch_en(csi->id, 1);
			if (i % 2 == 0) {
				csi->out_size.hor_len = sensor_format->width / 2 + vin_set_large_overlayer(sensor_format->width);;
				csi->out_size.ver_len = sensor_format->height;
				csi->out_size.hor_start = 0;
				csi->out_size.ver_start = 0;
			} else if (i % 2 == 1) {
				csi->out_size.hor_len = sensor_format->width / 2 + vin_set_large_overlayer(sensor_format->width);;
				csi->out_size.ver_len = sensor_format->height;
				csi->out_size.hor_start = sensor_format->width / 2 - vin_set_large_overlayer(sensor_format->width);;
				csi->out_size.ver_start = 0;
			}
		}
