...
static struct tdm_format *__tdm_try_format(unsigned int id)
{
	...
	if (global_video[id].large_image)
		tdm_rx->width = sensor_format->width / 2 + vin_set_large_overlayer(sensor_format->width);
	else
...
static int tdm_set_rx_cfg(struct tdm_rx_dev *tdm_rx, unsigned int en)
{
	...
	if (en) {
...
#if defined CONFIG_ARCH_SUN55IW3 || defined CONFIG_ARCH_SUN20IW3
				csic_tdm_rx_data_fifo_depth(tdm->id, tdm_rx->id, 32);
				csic_tdm_rx_head_fifo_depth(tdm->id, tdm_rx->id, 32);
#else
				csic_tdm_rx_data_fifo_depth(tdm->id, tdm_rx->id, tdm_rx->ws.data_fifo_depth);
				csic_tdm_rx_head_fifo_depth(tdm->id, tdm_rx->id, tdm_rx->ws.head_fifo_depth);
#endif
		...
		if (tdm_buf_num) {
			...
			tdm_rx->buf_size = size;
			if (size) {
#ifndef CONFIG_TDM_RX_BUF_NUM_WITH_TWORX
				if (tdm_buf_num == 1) {
					...
					for (i = 0; i < TDM_RX_BUFFER_FIFO - tdm_buf_num; i++) {
						csic_tdm_rx_set_address(tdm->id, tdm_rx->id, 0);
					}
				}
#else
				...
				for (i = 0; i < tdm_buf_num; i++) {
					if(tdm_rx->id == 1) {
						tdm_rx->buf_cnt = 0;
						tdm_rx->buf_size = 0;
						tdm_rx->buf[i].dma_addr = glb_tdm[0]->tdm_rx[0].buf[0].dma_addr;
						vin_warn("tdm_rx%d use tdm_rx%d buffer\n", tdm_rx->id, i);
					} else {
#ifdef CONFIG_KERNEL_FREERTOS
						tdm_rx->buf[i].dma_addr = memheap_alloc_align(&isp_mempool, size, 0x1000);
						vin_warn("tdm_rx%d use tdm_rx%d buffer\n", tdm_rx->id, i);
...
static int sunxi_tdm_top_s_stream(struct tdm_rx_dev *rx, int enable)
{
	...
	if (enable) {
		...
		hal_workqueue_cancel_work_sync(tdm->tdm_dev_run_task, &tdm->tdm_reset_work);
		if (tdm->work_mode == TDM_ONLINE) {
			csic_tdm_set_speed_dn(tdm->id, tdm->ws.speed_dn_en);
...
static void __tdm_reset_run_handle(hal_work *work, void* work_data)
{
	struct tdm_dev *tdm = (struct tdm_dev *)work_data;
	struct vin_core *vinc = NULL;
	struct tdm_rx_dev *tdm_rx = NULL;
	struct csi_dev *csi = NULL;
	struct isp_dev *isp = NULL;
	...
	for (k = 0; k < TDM_RX_NUM; k++) {
		...
		for (i = 0; i < VIN_MAX_DEV; i++) {
			...
			if (global_vinc[i]->stream_count == 0)
				continue;
			if (!tdm->tdm_rx_reset[global_vinc[i]->tdm_rx_sel])
				continue;

			tdm_rx = &tdm->tdm_rx[k];

			if (global_vinc[i]->tdm_rx_sel == tdm_rx->id) {
				vinc = global_vinc[i];

				isp = global_isp[global_vinc[i]->isp_sel];
				csi = global_csi[global_vinc[i]->csi_sel];
				bsp_isp_set_para_ready(isp->id, PARA_NOT_READY);
				csic_prs_capture_stop(vinc->csi_sel);
				...
				csic_tdm_rx_disable(tdm->id, vinc->tdm_rx_sel);
				csic_tdm_rx_cap_disable(tdm->id, vinc->tdm_rx_sel);
				vin_print("%s:workqueue tdm_rx%d reset!!!, isp frame count is %ld\n", __func__, vinc->tdm_rx_sel, isp->isp_frame_number);
				tdm_rx = &tdm->tdm_rx[vinc->tdm_rx_sel];
...
static hal_irqreturn_t tdm_isr(void *priv)
{
	...
	if (!tdm->tdm_task_resetting_flag && reset_task_flag) {
		tdm->tdm_task_resetting_flag = true;
		hal_workqueue_dowork(tdm->tdm_dev_run_task, &tdm->tdm_reset_work);
	}
...
int tdm_probe(unsigned int id)
{
	...
	snprintf(task_name, sizeof(task_name), "tdm_%d", id);

	tdm->tdm_dev_run_task = hal_workqueue_create(task_name, 16*1024, 7);
	if (!tdm->tdm_dev_run_task) {
		vin_err("Create tdm server work queue fail!\n");
		goto freedev;
	}
	hal_work_init(&tdm->tdm_reset_work, __tdm_reset_run_handle, tdm);
	...
	if (!tdm->base) {
		...
		if (ret != 0) {
#endif
			vin_err("tdm%d request tdm failed, ret:%d\n", tdm->id, ret);
			goto freedev;
		}
	}
...
int tdm_remove(unsigned int id)
{
	...
	hal_workqueue_destroy(tdm->tdm_dev_run_task);
	if (!tdm->is_empty) {
